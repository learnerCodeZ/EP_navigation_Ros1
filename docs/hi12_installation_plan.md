...
安装时注意：
- HI12 模块的坐标系方向应与 `base_link` 对齐，具体对齐规则如下：

### 3.4 HI12 坐标系对齐

HI12 模块坐标系与 ROS `base_link` 坐标系的对齐关系：

| HI12 坐标轴 | 对应机器人方向 | 对应 ROS base_link 轴 |
|-------------|---------------|----------------------|
| 模块 X 轴 | 机器人前方 | +X |
| 模块 Y 轴 | 机器人左方 | +Y |
| 模块 Z 轴 | 机器人上方 | +Z |
...
```
  ...
  HI12 Z / ROS +Z：垂直模块平面向上
```

**安装时将模块平放（丝印朝上），模块箭头/丝印方向指向机器人前方**。这样对齐后：
- HI12 的欧拉角 Roll/Pitch/Yaw 可直接映射到 ROS 坐标系，无需额外旋转变换
- URDF 中 `imu_joint` 的 `rpy` 设为 `0 0 0` 即可
- 如果无法按此方向安装（例如只能侧放或箭头朝后），则需在 URDF 的 `imu_joint` 中设置对应的 `rpy` 旋转补偿

### 3.5 无需坐标变换：HI12 与 EP SDK 的区别

之前使用 EP 内置 IMU 时，驱动中需要做大量坐标映射（y 取反、yaw 取反、acc_y/acc_z 取反等），原因是 **RoboMaster SDK 使用了非标准坐标系**：

| 差异点 | EP SDK 坐标系 | ROS REP-103 标准 |
|--------|--------------|-----------------|
| y 轴方向 | 正=右 | 正=左 |
| yaw 方向 | 顺时针为正 | 逆时针为正 |

使用 HI12 外置 IMU 后，**不再受 SDK 坐标系影响**：

- HI12 是独立传感器，直接通过 UART 串口输出原始数据，不经过 EP SDK
- 只要硬件安装按照 3.4 节对齐，HI12 输出的 X/Y/Z 和各轴旋转方向天然符合 ROS REP-103 标准
- `hi12_imu_node.py` 驱动中**无需任何符号取反**，直接原值填入 `sensor_msgs/Imu` 即可
- 这从根本上消除了 SDK 坐标映射复杂度和潜在的映射错误风险
