...
### RPLIDAR A2 USB 接口与供电

RPLIDAR A2 套装含一个 Slamtec 适配器小盒，盒子上引出两根 USB 线：

| 接口 | 形状 | 作用 |
|------|------|------|
| 数据口 | Micro-B USB | 连接主机，传输激光扫描数据，同时给雷达核心板 5V 供电 |
| 电机供电口 | Type-A USB（圆头） | 专门给雷达电机提供额外 5V 电源 |

**为什么有两个口**：RPLIDAR A2 电机启动瞬间电流较大（约 500mA），单口 USB 供电可能不足，导致电机不转、驱动报 `exit code -11`（SIGSEGV）或扫描数据异常。电机供电口用于补足电流。

**接法建议**：
- **两根都连到 Jetson**：两路 USB 并联供电，多数情况能正常工作
- 若仍供电不足（电机启动困难、扫描卡顿）：把圆头那根改接到独立 5V 电源适配器（如手机充电头），数据口仍连 Jetson

> Jetson Nano USB 口供电能力有限，是导致雷达电机不转的常见原因。优先用 Jetson 的 USB 3.0 口（蓝色）或带独立供电的 USB Hub。
...
当前仅在室内单一环境测试过建图和导航。换不同环境（走廊、大空间、多房间）验证 SLAM 稳定性和 AMCL 重定位成功率。

### 磁力计校准

HI12 室内静态漂移测试 5 分钟 0°，表现极佳，暂不需要校准。但如果换到金属结构多的环境（工厂、机房），磁力计可能受干扰，需用 HiPNUC 上位机做校准。校准后如果仍不稳定，可在 HI12 上位机中关闭磁力计，EKF 配置改回 `imu0_relative: true`。

### TEB 精细调参

当前参数解决了振荡和超时问题，但仍有优化空间：

| 参数 | 当前值 | 可调方向 | 效果 |
|------|--------|----------|------|
| `max_vel_x` | 0.8 m/s | 降到 0.4~0.6 | 低速更稳，减少打滑 |
| `max_vel_theta` | 1.0 rad/s | 降到 0.5~0.8 | 减少转弯顿挫 |
| `weight_optimaltime` | 1.0 | 调高 | 路径更短更快 |
| `weight_viapoint` | 1.0 | 调高 | 路径跟踪更紧 |
| `dt_ref` | 0.1 | 调到 0.15~0.2 | 减少轨迹点数，降低计算量 |

### 导航朝向精度

当前 `yaw_goal_tolerance: 0.30rad(17°)`，到点后朝向可能偏十几度。如果需要精确朝向（如对准充电桩、对准目标物体），可收紧到 0.1rad(5.7°)，但可能重新出现振荡，需配合增大 `controller_frequency` 或降低到达速度。

### EP 连接稳定性

EP SDK 偶尔出现 `固件版本: None` 和 `send_sync_msg timeout`，不影响运行但不够稳健。可在驱动中加重试逻辑：连接失败后等 5 秒重试，最多 3 次。
